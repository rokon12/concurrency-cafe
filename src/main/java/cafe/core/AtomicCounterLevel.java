...
public final class AtomicCounterLevel implements Level {
    ...
    public Outcome validate(SimulationResult sim) {
        ...
        if (actual == EXPECTED) {
            return new Outcome(true, "Counter ended at " + actual + " — atomic ops, no lock needed.", sim, List.of());
        }
        return new Outcome(false, "Counter ended at " + actual + ", expected " + EXPECTED + ".", sim, List.of());
    }
}
